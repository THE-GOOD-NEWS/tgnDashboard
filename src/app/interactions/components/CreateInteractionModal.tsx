...
const CreateInteractionModal = ({
  ...
  const [formData, setFormData] = useState({
    ...
    parentType: "video",
    parentId: "",
    targetType: "video" as "video" | "comment" | "reply",
    actionType: "like" as "like" | "unlike" | "comment" | "reply",
    targetId: "",
    content: "",
    read: false,
  });
    ...
    const fetchData = async () => {
      try {
        const [usersRes, videosRes] = await Promise.all([
          axios.get(`/api/users?all=true`),
          axios.get(`/api/videos?all=true`),
        ]);
  ...
  useEffect(() => {
    if (formData.targetType === "video" && formData.parentId) {
      setFormData((prev) => ({ ...prev, targetId: prev.parentId }));
    }
  }, [formData.targetType, formData.parentId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Failed to create interaction");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-999 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-full max-w-xl rounded-sm border border-stroke bg-white p-5 shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="For video targets, this auto-fills from Parent"
              className="w-full rounded-md border border-stroke bg-white py-3 px-4 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary"
            />
          </div>
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-md border border-stroke bg-white py-3 px-4 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary"
              placeholder="Optional; required for comments/replies"
            ></textarea>
          </div>
